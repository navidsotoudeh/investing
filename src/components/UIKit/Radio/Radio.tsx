			...
			<div className="flex gap-4">
				{radioOptions?.map((option, index) => {
                 ...
                 ${
                  ...
                  ${items.filter((ele: any) => ele.id === index)[0]?.isChecked ? 'bg-red-600' : ''}
                  ${
                    ...
                    ${isDisabled && items.filter((ele: any) => ele.id === index)[0]?.isChecked ? 'bg-surface-50' : ''}
                
                         `}
							></div>
							<span className={`${isDisabled ? 'text-text-20' : 'text-teal-60'}`}>{option?.label}</span>
						</div>
					);
				})}
			</div>
